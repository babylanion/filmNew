...
angular.module("app")
    ...
    .directive("datepicker", ["$locale", "$location", "$filter", "datePickerUtils", function ($locale, $location, $filter, dateUtils) {
        return {
            ...
            '<div class="{{d.className}}" ng-class="toBottom($index, dates[month].length)">' +
            ...
            link: function (scope, element, attrs, ngModel) {
                ...
                scope.toBottom = function (index, length) {
                    if (index + 1 == length)
                ...
                scope.render = function () {
                    ...
                    angular.forEach(scope.monthsArray, function (item, j) {
                        ...
                        for (var i = 0; i < allDates.length; i++) {
                            ...
                            // 大于enddate的灰掉, 小于startDate的灰掉
                            if (date > scope.endDate || date < scope.startDate) {
                                className = "pickadate-hide pickadate-disabled";
                            }

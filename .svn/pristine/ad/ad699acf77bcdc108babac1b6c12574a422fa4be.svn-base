...
angular.module('app')
    .controller('PreStudiosCtrl',["$scope", "$state", "$rootScope","$cookies", "movieService", "$filter", "calculate", function ($scope, $state, $rootScope,$cookies, movieService, $filter, calculate) {
        ...
        $scope.getpresellCinema = function () {
            ...
            movieService.presellCinema($scope.params)
                .error(function(){
                ...
                if($scope.preStudios == undefined || $scope.preStudios.results == null || $scope.preStudios.results.cinemaList == undefined || $scope.preStudios.results.cinemaList == null || $scope.preStudios.results.cinemaList.length == 0){
